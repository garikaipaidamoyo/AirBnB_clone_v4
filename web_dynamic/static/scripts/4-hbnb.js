$(document).ready(function() {
	...
	functon updateAmenitieslist() {
		...
		$('div.amenities h4').txt('Amenities: ' + amenitiesList);
	}
	...
	$('button').click(function() {
		// Get the list of selected Amenity IDs
		...
		// Make a new POST request to places_search endpoint with selected Amenities
		fetch('http://0.0.0.0:5001/api/v1/places_search/', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
		...
		.then(response => console.error('Error fetching places:', error));
	});
});
